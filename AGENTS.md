...
- `support/code/`：求解代码。
- `support/results/pX/`：第 X 问计算结果输出目录，论文中的数值必须从这里读取或复核。
- `figures/`：论文图表输出目录，图表文件名应能对应问题编号。
- `sections/problemX.tex`：第 X 问 LaTeX 正文文件。
- `docs/`：Codex 工作计划、模型说明、审查记录。
- `tests/`：格式检查、结果一致性检查脚本。

## 基本工作纪律

1. 每个阶段开始前必须先运行 `git status --short`，确认当前工作区状态。
2. 每个阶段开始前必须说明本阶段计划、涉及文件、验证方式。
3. 不允许修改 `support/data/` 原始附件。
4. 不允许凭空编造数值。论文中的所有指标、成本、产量、购电、售电、利用率必须来自 `support/results/pX/`。
5. 不允许只写论文不跑代码；不允许只跑代码不检查论文一致性。
6. 如果模型失败、结果异常或指标明显不合理，必须先记录问题，再最小修改模型或代码。
7. 每个问题的结果必须能通过脚本复现。
8. 每次完成一个阶段后，必须运行 `git diff --stat` 和 `git status --short`。
9. 阶段通过验证后，必须创建 Git commit；不要自动 push。
10. 如果验证失败，不要 commit 成功状态；可以 commit 为 `wip:`，但必须在提交信息中说明失败项。

## 建模边界与递进策略

问题二、三、四必须采用递进策略，三个问题不要混在一起做：
- 问题二作为离散开停机制氨优化基线。不得引入储能。
- 问题三作为连续功率可调优化。维持并网基础，不得混入离网储能配置。
- 问题四作为离网运行与储能配置扩展。
