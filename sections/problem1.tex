...
和 $P_t^{\mathrm{sell}}$。忽略园区功率损耗后，各功率满足
\begin{equation}
...
\end{equation}
式~\eqref{eq:p1-grid-exchange} 将新能源余量对应到上网功率，将功率
缺额对应到网购功率。因此，本问题不涉及生产时段选择和功率调节，
固定负荷方案主要作为后续优化问题的基准。

新能源自发自用电量记为
$E^{\mathrm{self}} = E^{\mathrm{re}} - E^{\mathrm{sell}}$。为与题面三项
绿电直连指标的电量边界保持一致，本文将指标中的``总用电量''按宽口径
定义为
$E^{\mathrm{total}} = E^{\mathrm{re}} + E^{\mathrm{buy}}
= E^{\mathrm{self}} + E^{\mathrm{sell}} + E^{\mathrm{buy}}$，即新能源
总发电量与网购电量之和；园区实际内部用电量另记为
$E^{\mathrm{use}}$。在该口径下，三项绿电直连指标分别描述新能源
就地消纳程度、总用电量绿电比例和新能源余电上网比例，其计算式为:
\begin{align}
  \Pi_{\mathrm{self}}
  ...
  \label{eq:indicators}
\end{align}
式~\eqref{eq:indicators} 分别对应新能源就地消纳、宽口径总用电量
中的绿电占比和对公共电网反送电的约束。其中三项指标需要分别满足
$\Pi_{\mathrm{self}} > 60\%$、$\Pi_{\mathrm{green}} > 30\%$、
$\Pi_{\mathrm{sell}} < 20\%$。

为评价固定运行方案的经济性，将风电、光伏度电平准化成本，
电氢氨装备运维费用、合成氨装置固定投资折算成本、分时购电费用以及余电上网收入统一计入日运行
...
电价与余电上网电价，$C_{\mathrm{NH_3}}^{\mathrm{fix}}$ 为合成氨装置投资
按寿命折算到典型日的固定成本；余电上网收入以负成本形式抵扣日运行成本。
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{p1_indicator_thresholds.pdf}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{p1_cost_breakdown.pdf}
...
可见，
固定运行方案下新能源自发自用比例与宽口径总用电绿电比例均满足要求，
但新能源上网比例达到 $35.92\%$，超过 $20\%$ 上限。

造成该结果的主要原因并非典型日总电量不足，而是风光出力的峰谷波动与
...
157\,270.83~元，折合吨氨成本为 \textbf{4\,368.63~元/t}。
因此，问题一结果表明，固定满负荷方案难以同时降低购电依赖与余电
