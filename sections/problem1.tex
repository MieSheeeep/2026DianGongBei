...
新能源自发自用电量记为
$E^{\mathrm{self}} = E^{\mathrm{re}} - E^{\mathrm{sell}}$。按照题目给出的
指标公式，``总用电量''采用指标核算口径：
$E^{\mathrm{total}} = E^{\mathrm{re}} + E^{\mathrm{buy}}
= E^{\mathrm{self}} + E^{\mathrm{sell}} + E^{\mathrm{buy}}$。
该口径不同于园区内部实际用电量
$E^{\mathrm{use}} = E^{\mathrm{self}} + E^{\mathrm{buy}}$，后文同时列出
二者以避免混淆。
三项绿电直连指标分别刻画新能源就地消纳程度、园区用电绿电占比和
新能源余电上网比例，其计算式为:
\begin{align}
  \Pi_{\mathrm{self}}
  &= \frac{E^{\mathrm{total}}-E^{\mathrm{sell}}-E^{\mathrm{buy}}}{E^{\mathrm{re}}}
   = \frac{E^{\mathrm{self}}}{E^{\mathrm{re}}}, &
  \Pi_{\mathrm{green}}
  &= \frac{E^{\mathrm{re}}-E^{\mathrm{sell}}}{E^{\mathrm{total}}}
...
电价与余电上网电价。由于电量以 MWh 计、电价以元/kWh 计，式中
用 $10^3$ 完成单位换算；余电上网收入以负成本形式抵扣日运行成本。

在题设固定运行条件下，
$P^{\mathrm{ALK}}_t \equiv P^{\mathrm{PEM}}_t \equiv 10$~MW、
$P^{\mathrm{NH_3}}_t \equiv 0.75$~MW，制氨速率
$m^{\mathrm{NH_3}}_t \equiv 1.5$~t/h，因此园区典型日制氨量为
36~t。计算中取 $\Delta t = 1$~h，风电与光伏度电平准化成本分别为
0.15~元/kWh 和 0.12~元/kWh，ALK、PEM 与合成氨装置运维系数分别为
0.10、0.15 与 0.002~元/kWh，余电上网电价为 0.3779~元/kWh，
分时购电电价按附件 7 取值。
...
总功率峰值约为 60.6~MW，明显高于满负荷运行时约 24~MW 的园区用电
...
\begin{table}[H]
  ...
  \begin{tabular}{lc}
    ...
    指标口径总用电量 $E^{\mathrm{total}}$ & 775.49 \\
...
园区内部用电量为 558.72~MWh，按题目公式核算的总用电量为
775.49~MWh。由于风光出力与固定生产负荷在时序上并不匹配，
园区在一天内同时发生 172.04~MWh 网购电与
