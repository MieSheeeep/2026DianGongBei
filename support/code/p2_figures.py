...
def plot_typical_schedule(summary: dict) -> Path:
    ...
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(7.2, 5.6), sharex=True, height_ratios=[3, 1])
    ax1.plot(hours, renewable, marker="o", linewidth=1.8, label=LABELS["renewable"])
    ax1.plot(hours, use, marker="s", linewidth=1.8, label=LABELS["use"])
    ax1.plot(hours, buy, linewidth=1.4, label=LABELS["buy"])
    ax1.plot(hours, sell, linewidth=1.4, label=LABELS["sell"])
    ax1.set_ylabel(LABELS["power"])
    ax1.set_title(f"{LABELS['typical_schedule_title']} ({best_prod:.0f} t/d)")
    ax1.grid(True, alpha=0.25)
    ax1.legend(ncol=2, fontsize=9)

    ax2.step(hours, on, where="mid", color="#444444", linewidth=1.6, label=LABELS["on"])
    ax2.fill_between(hours, 0, on, step="mid", color="#999999", alpha=0.25)
    ax2.set_yticks([0, 1])
...
def plot_cost_by_production(summary: dict) -> Path:
    ...
    ax.bar([n - width / 2 for n in x], typical_cost, width=width, label=LABELS["typical"])
    ax.bar([n + width / 2 for n in x], annual_cost, width=width, label=LABELS["annual_fixed"])
    ...
    ax.set_title(LABELS["cost_title"])
...
def plot_indicator_distribution() -> Path:
    ...
    for ax, key in zip(axes, ["self_use_ratio", "green_ratio", "sell_ratio"]):
        ...
        ax.errorbar(PRODUCTION_LEVELS, means, yerr=[lower, upper], marker="o", capsize=4, linewidth=1.6)
        ax.axhline(INDICATOR_THRESHOLDS[key], linestyle="--", color="#d62728", linewidth=1.2)
        ax.set_title(LABELS[key], fontsize=10)
        ax.set_ylim(0, 1.02)
        ax.grid(True, alpha=0.25)
        ax.set_xlabel(LABELS["production"])
    axes[0].set_ylabel(LABELS["ratio"])
    fig.suptitle(LABELS["indicator_title"], y=1.02)
...
def plot_annual_unit_cost_distribution(summary: dict) -> Path:
    ...
    ax.plot(range(1, len(costs) + 1), costs, marker="o", linewidth=1.6)
    ...
    ax.set_ylabel(LABELS["unit_cost"])
    ax.set_title(LABELS["annual_title"])
    ax.grid(True, alpha=0.25)
    ax.legend()
...
def plot_grid_exchange_distribution(summary: dict) -> Path:
    ...
    width = 0.34
    x = list(range(len(PRODUCTION_LEVELS)))
    ax1.bar([n - width / 2 for n in x], mean_buy, width=width, label=LABELS["buy_energy"])
    ax1.bar([n + width / 2 for n in x], mean_sell, width=width, label=LABELS["sell_energy"])
    ...
    ax2 = ax1.twinx()
    ax2.plot(x, annual_sell_ratio, color="#d62728", marker="o", linewidth=1.6, label=LABELS["sell_ratio_line"])
    ax2.axhline(INDICATOR_THRESHOLDS["sell_ratio"], linestyle="--", color="#d62728", alpha=0.6, linewidth=1.0)
    ax2.set_ylabel(LABELS["ratio"])
    ax2.set_ylim(0, max(annual_sell_ratio) * 1.25)

    handles1, labels1 = ax1.get_legend_handles_labels()
    handles2, labels2 = ax2.get_legend_handles_labels()
    ax1.legend(handles1 + handles2, labels1 + labels2, loc="upper center", ncol=3, fontsize=9)
    ax1.set_title(LABELS["grid_title"])
