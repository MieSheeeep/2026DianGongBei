...
- 本阶段只完成建模计划和模型互审，不写 `p3_solve.py`，不生成结果，不画图，不修改论文正文。
...
- `P_alk[s,t,q]`：ALK 电解槽功率，MW。
- `P_pem[s,t,q]`：PEM 电解槽功率，MW。
- `m_nh3[s,t,q]`：合成氨产量，t/h。
- `P_nh3[s,t,q]`：合成氨装置功率，MW。
- `P_buy[s,t,q]`：购电功率，MW。
- `P_sell[s,t,q]`：上网功率，MW。
- `P_curtail[s,t,q]`：弃电功率，MW。并网场景主口径下取 0 或仅作平衡审计字段。

主方案采用连续线性规划，不引入二元变量。原因是题面问题三强调“功率连续可调”，与问题二“只有全额开机和停机”形成对照；若引入二元变量表示开停状态，则模型会变为“可停机的连续调节 MILP”，会削弱问题二、三的递进关系。

下限 10% 的主处理方式为：连续可调装置在 24 小时均保持在线运行，功率位于额定值的 10%--100% 范围内。若后续审查认为“可以停机，且开机时最低 10%”才符合题意，则需要引入二元在线状态变量 `y[t]`，并使用 `0 或 [10%,100%]` 的混合整数约束；该方案列为备选，不进入阶段 B 主实现，除非用户明确要求。
...
= P_load[s,t] + P_alk[s,t,q] + P_pem[s,t,q] + P_nh3[s,t,q]
...
```text
0.1 * P_alk_max <= P_alk[s,t,q] <= P_alk_max
0.1 * P_pem_max <= P_pem[s,t,q] <= P_pem_max
```
...
```text
0.3 <= m_nh3[s,t,q] <= 3.0       t/h
P_nh3[s,t,q] = 0.5 * m_nh3[s,t,q] MW
```
...
不引入氢储存和跨时段库存，逐小时制氢量与合成氨耗氢量平衡。由初始设备参数可得：

- ALK：10 MW 对应 140 kg/h，因此产氢系数为 14 kg/(MWh)。
- PEM：10 MW 对应 160 kg/h，因此产氢系数为 16 kg/(MWh)。
- 合成氨耗氢：0.2 kg H2/kg NH3，即 200 kg H2/t NH3。

逐小时平衡为：

```text
14 * P_alk[s,t,q] + 16 * P_pem[s,t,q]
= 200 * m_nh3[s,t,q]
```

该式保证连续调节下制氢和制氨同步，不把问题四的储能或氢储存提前引入问题三。
...
- `p3_continuous_schedule.pdf`：代表场景连续调度曲线，展示风光出力、ALK/PEM/合成氨功率、购电、上网和产氨量。
...
若当前 `tests/check-p3-results.ps1` 尚未检查 `p3_daily_cases.csv`、氢平衡、10% 下限、净成本公式和指标公式，应在代码阶段同步增强测试脚本，但不得修改题面数据。
...
2. 对每个场景和每个 `q` 建立连续线性规划。
3. 若可用 `scipy.optimize.linprog`，使用线性规划求解；若环境中没有 SciPy，则可先检查 `learn3.8` 环境是否可用，或实现一个等价的约束矩阵求解接口。
4. 输出逐时与日汇总结果。
...
- 氢平衡不能只靠设备同步比例隐含。修正：第 7.5 节显式写入 ALK、PEM 产氢和合成氨耗氢逐小时平衡。
- 官方绿电比例不得误用内部用电绿电比例。修正：第 8 节明确 `green_internal_use_ratio` 仅为补充审计字段。

### 14.3 optional improvements

- 后续代码阶段可增强 `tests/check-p3-results.ps1`，增加 `p3_daily_cases.csv`、氢平衡、10% 下限、指标公式和净成本公式检查。
- 图表阶段可选择一个典型高风光场景与一个低风光场景，展示连续调节对购售电曲线的改善，而不只展示全年汇总图。

修正后，问题三建模计划可进入代码实现阶段；本阶段仍不写求解代码、不生成结果、不画图、不修改论文正文。
