...
def solve_case(
    ...
    daily_cost = _sum([float(row["cost_yuan"]) for row in hourly_rows]) + NH3_FIXED_CAPEX_DAILY_YUAN
    daily_revenue = _sum([float(row["sell_revenue_yuan"]) for row in hourly_rows])
    daily_net_cost = _sum([float(row["net_cost_yuan"]) for row in hourly_rows]) + NH3_FIXED_CAPEX_DAILY_YUAN
    ...
    daily_row: dict[str, float | int | str] = {
        ...
        "daily_nh3_fixed_capex_yuan": _round_float(NH3_FIXED_CAPEX_DAILY_YUAN),
